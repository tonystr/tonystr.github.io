...
KanjiResults.prototype.overflow_button = function() {
    ...
    if (!this.silly_remaining && remaining > 0) {
        ...
        var downbutton = create_text_node("a", this.down_button, "â–¼" + below + " more");
    }
    this.clear_up_button();
    if (this.offset != 0) {
        var upbutton = create_text_node("a", this.up_button, "â–²" + above + " more");
    } else {
        if (!this.silly_remaining && remaining > 0) {
            upbutton = create_text_node("a", this.up_button, "â–² 0 more");
...
function clear_link_preference() {
    delete_cookie("link");
...
function create_link(parent, kanji, window_preference) {
    if (use_input_box) {
        ...
        write_kanji_el.className = "write_kanji";
        write_kanji_el.onclick = function() {
            window.parent.postMessage(kanji, "*");
        }
    } else {
        var make_link = true;
        var link_preference = get_link_preference();
        ...
        if (make_link) {
            var href = 'minidic.cgi?r=' + encodeURIComponent(kanji);
...
function search_input_box() {
    ...
    var href = 'minidic.cgi?r=' + encodeURIComponent(search_string);
    var window_preference = get_window_preference();
    if (window_preference == 1) {
        window.open(href);
    } else {
